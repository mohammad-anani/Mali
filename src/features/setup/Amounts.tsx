...
export default function Amounts({ object, setObject }: { object: initObject; setObject: Setter<initObject> }) {
  ...
  const lbpRef = useRef<TextInput>(null);
  const usdRef = useRef<TextInput>(null);

  return <View className='gap-3'>
    <Text className='text-[35px] text-secondary ml-5 '>Enter your balances:</Text>
    <View className='gap-3 '>
      <View className='flex flex-row gap-3'>
        <Input ref={lbpRef} keyboardType="numeric" placeholder='25,000,000' className='w-[75%]' value={object.lbp ? object.lbp.toString() : undefined} onChangeText={handleLBPChangeText} returnKeyType='next' onSubmitEditing={() => usdRef.current?.focus()} />
      ...
      <View className='flex flex-row gap-3 '>
        ...
        <Input ref={usdRef} keyboardType="numeric" placeholder='5,000' className='w-[75%] ' value={object.usd ? object.usd.toString() : undefined} onChangeText={handleUSDChangeText} returnKeyType='done' />
