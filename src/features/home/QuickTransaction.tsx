...
export default function QuickTransaction({ isDeposit = false, balances }: { isDeposit?: boolean, balances: [number | null, number | null] }) {
  ...
  const { mode, object, setObject, hasSubmitted, isSubmitError, submit, isLoading } = useQuickTransaction(isDeposit, balances);
    ...
    <View>
      ...
      <ContentView className='gap-2' >
        ...
        <View className='gap-3'>
          ...
          <AmountInput inputRef={amountRef} amount={object.amount} isLBP={object.isLBP} setAmount={(a) => { setObject(o => ({ ...o, amount: a })) }} setIsLBP={(c) => { setObject(o => ({ ...o, isLBP: c })) }} hasSubmitted={hasSubmitted} balances={balances} isDeposit={isDeposit} />

          <View className='flex-row justify-end'>
            <Button pressableProps={{ className: `${color} rounded-full w-[190px] h-[60px] justify-center disabled:bg-muted`, disabled: isSubmitError, onPress: submit }} textProps={{ className: "text-[30px] text-center text-secondary" }}>{mode}</Button>
          </View>
        </View>
      </ContentView>
    </View>
